...
$bdMyServices = mysqli_query($connect, "SELECT * FROM `creatingAservice` 
INNER JOIN `users` ON creatingAservice.user = users.id_user 
INNER JOIN `unitMeasurement` ON creatingAservice.id_unit = unitMeasurement.id_unit 
INNER JOIN `podkategorii` ON creatingaservice.categories = podkategorii.id_podkategorii 
INNER JOIN `kategoria` ON Kategorii = kategoria.Id WHERE user = '$idUser'");
...
<body>
    ...
    <div class="modalCreatingAservice" onclick="createAserviceModal()">
        <div onclick="event.stopPropagation()">
            <form onsubmit="return volidateFormCreate()" class="modalCreatingAserviceWindow" name="creatingAservice"
                action="bdСreatingAservice.php" method="post" enctype="multipart/form-data">
                ...
                <div class="userInfoModal">
                    ...
                    <div class="userInfoModalRight">
                        ...
                        <p class="modalUserProfileWindowInfoListMail">+7
                            <?= $delFirstCharacter ?>
                        </p>
    ...
    <div class="modalCatigories" onclick="modalCatigories()">
        <div class="modalCatigoriesWindow" onclick="event.stopPropagation()">
            ...
            <?php
            $selectOwnerCategory = mysqli_query($connect, "SELECT * FROM `kategoria`");
            $selectUnderCategory = mysqli_query($connect, "SELECT * FROM `podkategorii` WHERE `Kategorii` = '{$_SESSION['underCategory']}'");

            ?>
            <div class="modalCatigoriesWindowContainer">
                <div class="modalCatigoriesWindowLeft" alink=green>
                    <?php foreach ($selectOwnerCategory as $inputOwnerCategory): ?>
                        <a href="categories.php?id_ownerCat=<?php echo $inputOwnerCategory['Id'] ?>"><?php echo $inputOwnerCategory['Name'] ?></a>
                    <?php endforeach; ?>
                </div>
                <div class="modalCatigoriesWindowRight">
                    <?php foreach ($selectUnderCategory as $underCategory): ?>
                        <a href="nameCategories.php?description=<?= $underCategory['id_podkategorii'] ?>">
                            <?= $underCategory['PodKategoria'] ?>   
                        </a>
                    <?php endforeach; ?>
                </div>
            </div>
            ...
            <form method="post">
                <?php foreach ($bdMyServices as $key => $myServices): ?>
                    <div class="MyOrder">
                        ...
                        <?php
                        $response = mysqli_query($connect, "SELECT * FROM `response` 
