...
function validateFormReg() {
    ...
    if (name == "") {
        ...
        let regex = /[^-А-ЯA-Z\x27а-яa-z][^\d\+]/g;
    ...
    if (number == "") {
        printError("numberErr", "Пожалуйста, введите корректно ваш номер телефона");
    } else {
        let regex = /^\d{11}$/;
    ...
    if (email == "") {
        printError("emailErr", "Пожалуйста, заполните введите корректную почту");
    } else {
        let regex = /^\S+@\S+\.\S+$/;
    ...
    if (description == "") {
        printError("descriptionErr", "Пожалуйста, заполните кратко описание");
    } else {
